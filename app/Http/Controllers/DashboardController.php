...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        // Si es ADMIN, $idsAreas será null para indicar alcance global
        $idsAreas        = $esAdmin ? null : $user->getIdsAreasPermitidas();   // array de IDs
        ...
        for ($i = 6; $i >= 0; $i--) {
            ...
            // Movimientos del día (usar helper para respetar filtro de áreas)
            $qMov = PermisosHelper::getMovimientosQuery()->whereDate('fecha_mvto', $dateStr);
            $totalMov = $qMov->count();
        ...
        // Usar helper para respetar permisos por área
        $qMesActual   = PermisosHelper::getMovimientosQuery()->whereMonth('fecha_mvto', now()->month)->whereYear('fecha_mvto', now()->year);
        $qMesAnterior = PermisosHelper::getMovimientosQuery()->whereMonth('fecha_mvto', now()->subMonth()->month)->whereYear('fecha_mvto', now()->subMonth()->year);
    ...
    public function getStats()
    {
        $user    = Auth::user();
        $esAdmin = $user->esAdmin();
        $idsAreas = $esAdmin ? null : $user->getIdsAreasPermitidas();

        // Total de bienes: usar PermisosHelper para respetar permisos
        $totalBienes = PermisosHelper::getBienesQuery()->where('activo', true)->count();

        // Movimientos hoy: usar helper para filtrar por áreas si aplica
        $qHoy = PermisosHelper::getMovimientosQuery()->whereDate('fecha_mvto', today());

        // Último movimiento respetando permisos
        $ultimoMovimiento = PermisosHelper::getMovimientosQuery()->with('bien')->latest('fecha_mvto')->first();
