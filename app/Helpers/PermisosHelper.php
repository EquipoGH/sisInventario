...
class PermisosHelper
{
    ...
    {
        $user = Auth::user();
        // ⭐⭐⭐ Solo ADMIN ve todos los bienes ⭐⭐⭐
        if (self::esAdmin()) {
            return Bien::query();
        }

        // Si el usuario no tiene responsable asignado, no ve nada
        if (empty($user->id_responsable)) {
            return Bien::whereRaw('1 = 0');
        }

        // TODOS LOS DEMÁS filtran por área (incluye INFORMATICA y otros roles)
        ...
        // O bienes que ellos mismos registraron
        return Bien::where(function($q) use ($areasPermitidas, $user) {
            ...
            ->orWhere('registrado_por', $user->id);
        });
    }
    ...
    public static function getMovimientosQuery()
    {
        ...
        // Si el usuario no tiene responsable asignado, no ve nada
        if (empty($user->id_responsable)) {
            return \App\Models\Movimiento::whereRaw('1 = 0');
        }
